...
CMD ["sh","-c", "jupyter lab --notebook-dir=/home/jovyan --ip=0.0.0.0 --no-browser --allow-root --port=8888 --NotebookApp.token='' --NotebookApp.password='' –NotebookApp.tornado_settings='{"headers":{"Content-Security-Policy":"frame-ancestors self   http://192.168.1.229:8082;" }}' --NotebookApp.allow_origin='*' --NotebookApp.base_url=${NB_PREFIX}"]
